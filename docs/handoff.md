...
- **全量结果**(管理员放行,train+val 串行 ~15 分钟,~3,193 次调用零失败):
  train 2,445 条 = pass 1,461 / fixed 960 / human 24;val 736 = pass 466 /
  fixed 263 / human 7;合计 pass 1,927(60.6%)/ fixed 1,223(38.4%)/
  human 31(1.0%)。人工队列 ≈ 31 + 终审抽查。
- **overlay 定版(管理员拍板:AI 看整句+全貌,fix 默认采纳+出入标记;下游名单
  独立,上游 r5 不动)**:`build_assembly_session` 自动检测
  `outputs/ai_review/<run_tag>/ai_review.json` 并叠加——fixed 条目显示修正后
  query(人工编辑>AI修正>原句 优先级),条目带 ai_verdict/ai_reason;**同帧
  重复句碰撞标记**(AI 跨条目计数漂移,40 组/64 条,1.2%,全部人工关注);
  人工裁决条目(285)完全绕过 overlay。ai_review.json 即下游名单,烘焙 r6 的
  apply 待终审后按需执行。
- **前端**:画布框色收敛两态——灰=未人工过、亮绿=人工过(蓝/紫/橙全撤,待办
  等状态改文字表达);状态徽章带 AI 裁决(🤖 AI已过/✏️ AI已改/⚠️ AI转人工/
  ⚠同帧重复);Ctrl 家族(F/B/E/K/U)页面级拦截浏览器默认,Ctrl+A 文本框内
  保留全选;Ctrl+U 补齐 readline 行删对。
- **验证**:126 项测试全绿;node --check 通过;8791 临时会话实测 overlay
  字段与分布(见上)后即关闭。
- **下一步**:管理员合并会话快速终审(预期大幅提速)→ :todo 清单 →
  终审后按需 apply 烘焙 r6 → R6 打包链。

### 2026-09-07(AI 预审 pass 落地:run_ai_review.py + 审计合同定稿;真跑待管理员放行)
### 2026-09-07(AI 预审 pass 落地:run_ai_review.py + 审计合同定稿;真跑待管理员放行)

- **动因**:管理员人审 280+ 条后指出残留错误集中在序数/颜色/方向/歧义句,
  要求人审前加一层 AI 审计与纠正——教师当审计员(只判+受限修正),人工只接
  兜底。提示词经三轮对齐定稿:全景原图+目标加粗红框(不画编号框,管理员
  判断编号会诱导照抄标签),数数口径与普查同哲学(只数清晰可辨,忽略杂屑),
  修正词表锁死序数/方向/颜色三件(编辑不是重写),改后必须自查消歧否则判
  human,JSON 合同 {observed, verdict: pass|fixed|human, query, reason}。
- **落地**:`scripts/run_ai_review.py`——按条目分片(48 shards)+ 分片
  checkpoint 原地重写 + key 池/限流/断点续跑(对齐 run_enumeration 骨架);
  渲染复用 `build_marked_annotation_view`(全景+红框,1536 上限,零图片落仓);
  **人审过的条目按 review store journal 整条跳过**(人工裁决永不触碰);
  解析层强制 schema/verdict 白名单/pass-fixed 不一致自动矫正;UI 侧 Ctrl
  家族页面级拦截(浏览器查找/源码键)+ Ctrl+U 补齐 readline 行删对。
- **测试**:126 项全绿(+13:解析含 coercion、计划跳过人工、提示词合同钉死)。
- **preflight(零 API)**:train 2,730 条中人工已审 285 条跳过,待审 2,445;
  val 736 待审;图片 0 缺失;合计 ~3,181 次调用,prompt hash
  8a0a0e50…。真跑命令已就绪,**等管理员明确放行**;apply 步骤(fixed 重过
  唯一性门+桶分类器+文本QC → asm-*-r6)在真跑出数后实现。
- **下一步**:管理员放行真跑(建议先 --limit 20 冒烟)→ 检查 verdict 分布
  → apply → 人工队列只收 human。

### 2026-09-07(人审器:query 栏去标识、居中放大——打字优先布局)
### 2026-09-07(人审器:query 栏去标识、居中放大——打字优先布局)

- **动因**:管理员声明行退役后继续减负——query 栏的桶标签(序数/距离/方位/
  属性)也属冗余标识,且本轮审阅以打字为主、画框为辅,文字框应居中、字大。
- **改动**:`query-bucket-tag` 元素与 BUCKET_TAGS 渲染删除;`.query-en` 字号
  15→19px、文本居中、上限 900px;`.query-en-wrap` 居中对齐;`.query-meta`
  徽章列居中;无用的 `.query-label` 样式清理。画布上框旁的序数数字保留
  (多目标帧辨框仍需要),ID 徽章保留(图号/编号跳转入口)。
- **验证**:113 项测试全绿;node --check 通过;无 bucket-tag/claims 残留。
